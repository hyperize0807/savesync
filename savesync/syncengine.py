...
# 파일 전송 병렬 처리 워커 수. 파일당 네트워크 왕복(2~3초)이 병목이라, 동시에
# 여러 파일을 전송하면 크게 단축된다(Drive 사용자 쿼터 안에서 안전한 수준).
DEFAULT_WORKERS = 8
...
def sync_profile(
    ...
    # ── 1) 계획(순차) ──────────────────────────────────────────
    # 각 파일의 처리 방향을 먼저 정한다. 충돌 판정(특히 사용자 질문 ASK)과
    # skip 집계는 여기서 순차적으로 처리하고, 실제 전송만 병렬로 돌린다.
    # tasks: (kind, rel) — kind ∈ upload_new/download_new/overwrite_local/overwrite_drive
    ...
    for rel in all_rels:
        ...
        if in_local and not in_drive:
            tasks.append(("upload_new", rel))
        elif in_drive and not in_local:
            tasks.append(("download_new", rel))
        else:
            try:
            ...
            if direction == "skip":
                ...
                tasks.append(("overwrite_local", rel))
            else:  # "drive"
                tasks.append(("overwrite_drive", rel))

    # ── 2) 전송(병렬) ──────────────────────────────────────────
    # 파일당 네트워크 왕복이 병목이므로 여러 파일을 동시에 전송한다.
    # 반환값: (kind, rel, backed_up_bool, error_or_None)
    def _run(task: tuple[str, str]):
        kind, rel = task
        try:
            if kind == "upload_new":
                drive.upload_new(drive_folder_id, rel, local_files[rel])
                return (kind, rel, False, None)
            if kind == "download_new":
                _download(drive, drive_files[rel], local_root / rel)
                return (kind, rel, False, None)
            if kind == "overwrite_local":
                # 드라이브 버전을 백업 후, 로컬로 덮어쓰기
                data = drive.download_bytes(drive_files[rel]["id"])
                backed = bool(backup.backup_drive_bytes(data, rel))
                drive.update_existing(drive_files[rel]["id"], local_files[rel])
                return (kind, rel, backed, None)
            # overwrite_drive: 로컬 버전을 백업 후, 드라이브 내용으로 로컬 덮어쓰기
            backed = bool(backup.backup_local(local_files[rel], rel))
            _download(drive, drive_files[rel], local_root / rel)
            return (kind, rel, backed, None)
        except Exception as e:  # 한 파일 실패가 전체를 막지 않도록
            return (kind, rel, False, e)

    if tasks:
        workers = max(1, min(max_workers, len(tasks)))
        with ThreadPoolExecutor(max_workers=workers) as ex:
            # map 은 제출 순서대로 결과를 돌려주므로 로그 순서가 안정적이다.
            for kind, rel, backed, err in ex.map(_run, tasks):
                _apply_result(stats, kind, rel, backed, err, log)

    log(f"[{profile.name}] 완료 — {stats.summary()}")
    return stats


# 병렬 전송 결과를 stats 에 반영하고 로그를 남긴다(메인 스레드에서만 호출).
_RESULT_LOG = {
    "upload_new": "  ↑ 신규 업로드: {}",
    "download_new": "  ↓ 신규 다운로드: {}",
    "overwrite_local": "  ↑ 덮어쓰기(로컬→드라이브): {}",
    "overwrite_drive": "  ↓ 덮어쓰기(드라이브→로컬): {}",
}


def _apply_result(stats: "SyncStats", kind: str, rel: str, backed: bool,
                  err: Exception | None, log: Callable[[str], None]) -> None:
    if err is not None:
        msg = f"{rel}: {err}"
    ...
    elif kind == "download_new":
        stats.downloaded_new += 1
    elif kind == "overwrite_local":
        stats.uploaded_overwrite += 1
    elif kind == "overwrite_drive":
        stats.downloaded_overwrite += 1
    if backed:
        stats.backed_up += 1
    log(_RESULT_LOG[kind].format(rel))
...
def sync_all(
    ...
    for profile in cfg.profiles:
        ...
        if not profile.has_local_folder():
            ...
            continue
        results.append(
            sync_profile(drive, cfg, profile, log, on_conflict, max_workers))
    return results
